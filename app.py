...
def add_history_table(limit=20):
    with sqlite3.connect(DB_PATH, timeout=30) as conn:
        return pd.read_sql_query(f"""SELECT shop_name, bill_date, gst_number, total, calculated_total, status, timestamp FROM bills ORDER BY id DESC LIMIT {int(limit)}""", conn)
...
def open_naps2_scanner():
    if sys.platform != "win32":
        return False, "Windows only feature"
    naps2_paths = [r"C:\Program Files\NAPS2\NAPS2.exe", r"C:\Program Files (x86)\NAPS2\NAPS2.exe", r"C:\Program Files\NAPS2\naps2.exe", r"C:\Program Files (x86)\NAPS2\naps2.exe"]
...
def insert_bill(shop, date, gst, total, calc_total, status):
    with sqlite3.connect(DB_PATH, timeout=30) as conn:
        cur = conn.cursor()
        cur.execute("""INSERT OR IGNORE INTO bills (shop_name, bill_date, gst_number, total, calculated_total, status, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)""", (shop, date, gst, total, calc_total, status, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        conn.commit()
...
def render_upload_module(model_bundle):
    ...
    model_bundle = {"vision_client": vision_client, "gemini": gemini_model, "openai": openai_client, "textract_client": textract_client, "perplexity_key": perplexity_key}
...
def main():
    ...
    if not st.session_state.logged_in:
        do_login()
    with st.sidebar:
        st.markdown("""<div class="sidebar-brand-box"><div class="sidebar-title">Deep CSC</div><div class="sidebar-subtitle">AI Bill Processor</div><div class="sidebar-id-badge">ID: 256423250015</div></div>""", unsafe_allow_html=True)
        st.write("Provider-ready OCR and invoice extraction dashboard.")
        if st.button("Logout", use_container_width=True):
            terminate_session()
    render_upload_module({"vision_client": setup_google_vision(), "gemini": setup_gemini(), "openai": setup_openai(), "textract_client": setup_textract(), "perplexity_key": secret_or_default("PERPLEXITY_API_KEY", "").strip()})
