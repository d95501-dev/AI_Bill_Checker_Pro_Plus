...
def process_single_image(
    ...
    try:
        ...
        text = perform_ocr(
            image=image,
            provider=provider,
            gemini_model=model_bundle.get(
                "gemini"
            ),
            openai_client=model_bundle.get(
                "openai"
            ),
            vision_client=model_bundle.get(
                "vision"
            )
        )
...
def process_pdf_file(
    ...
    return process_images_parallel(
        images,
        provider,
        model_bundle
    )
...
def render_analytics_dashboard(df):
    ...
    st.success(
        f"Valid GST: {gst_data['valid']}"
    )

    st.warning(
        f"Invalid GST: {gst_data['invalid']}"
    )
import shutil
